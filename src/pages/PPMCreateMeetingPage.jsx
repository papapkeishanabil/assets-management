...
export default function PPMCreateMeetingPage() {
  ...
  const addPOItem = () => {
    ...
    setPoItems([...poItems, { id: newId, po_number: '', customer_name: '', project_name: '', description: '', deadline: '', document: null, documentPreview: null, uploading: false }]);
  };
  ...
  const handleDocumentChange = async (e, poId) => {
    ...
    setPoItems(prevItems => prevItems.map(item => item.id === poId ? { ...item, uploading: true } : item));
    
    try {
      console.log('Uploading file for PO id:', poId, 'File:', file.name);
      const uploadResult = await uploadPPMDocument(file);
      console.log('Upload result:', uploadResult);
      const preview = file.type.startsWith('image/') ? URL.createObjectURL(file) : null;
      
      // Use functional update to get the latest state
      setPoItems(prevItems => prevItems.map(item => item.id === poId ? { ...item, document: uploadResult, documentPreview: preview } : item));
      ...
      // Use functional update to get the latest state
      setPoItems(prevItems => prevItems.map(item => item.id === poId ? { ...item, uploading: false } : item));
    }
  };
  ...
  const validateForm = () => {
    ...
    console.log('Validating poItems:', poItems);
    for (const item of poItems) {
      console.log('Validating PO:', item.id, 'po_number:', item.po_number, 'customer:', item.customer_name, 'document:', item.document);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('Inserting meeting:', meetingInsert);
      
      const { data: meetingData, error: meetingError } = await supabase.from('ppm_meetings').insert([meetingInsert]).select('id').single();
      console.log('Meeting insert result:', { meetingData, meetingError });
      ...
      if (!meetingData || !meetingData.id) {
        throw new Error('Meeting ID not returned from insert');
      }
      ...
      console.log('Navigating to:', '/ppm/' + meetingId);
      navigate('/ppm/' + meetingId);
    } catch (error) {
      console.error('Error creating meeting:', error);
      toast.error('Gagal membuat meeting: ' + error.message);
    } finally {
      setSaving(false);
    }
  };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input 
                      id={`file-${item.id}`}
                      type="file" 
                      onChange={(e) => handleDocumentChange(e, item.id)} 
                    ...
                    <label 
                      ...
                      disabled={saving || item.uploading}
                    >
